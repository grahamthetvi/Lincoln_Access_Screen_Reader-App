...
class GeneralSettingsPanel(SettingsPanel):
	...
	def makeSettings(self, settingsSizer):
		...
		item = self.autoCheckForUpdatesCheckBox = wx.CheckBox(
			self,
			# Translators: The label of a checkbox in general settings to toggle automatic checking for updated versions.
			# If not checked, user must check for updates manually.
			label=_("Automatically check for &updates"),
		)
...
class AddonStorePanel(SettingsPanel):
	...
	def makeSettings(self, settingsSizer: wx.BoxSizer) -> None:
		sHelper = guiHelper.BoxSizerHelper(self, sizer=settingsSizer)
		self.enableRemoteCatalog = sHelper.addItem(
			wx.CheckBox(
				self,
				# Translators: Option to allow the Add-on Store to contact the network for catalog metadata.
				label=_("Allow &network access for the add-on store catalog"),
			),
		)
		self.enableRemoteCatalog.SetValue(config.conf["addonStore"]["enableRemoteCatalog"])
	...
	def onSave(self):
		config.conf["addonStore"]["enableRemoteCatalog"] = self.enableRemoteCatalog.IsChecked()
