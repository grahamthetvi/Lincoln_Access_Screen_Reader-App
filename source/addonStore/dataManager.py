...
class _DataManager:
	...
	def __init__(self):
		...
		# Fetch available add-ons cache early (NV Access catalog; optional for managed/offline builds).
		self._initialiseAvailableAddonsThread: Optional[threading.Thread] = None
	...
	def terminate(self):
		...
		if self._initialiseAvailableAddonsThread is not None and self._initialiseAvailableAddonsThread.is_alive():
	...
	def _getLatestAddonsDataForVersion(self, apiVersion: str) -> Optional[bytes]:
		if not config.conf["addonStore"]["enableRemoteCatalog"]:
			return None
	...
	def getLatestCompatibleAddons(
		...
		if not config.conf["addonStore"]["enableRemoteCatalog"]:
			if self._compatibleAddonCache is not None:
	...
	def getLatestAddons(
		...
		if not config.conf["addonStore"]["enableRemoteCatalog"]:
			if self._latestAddonCache is not None:
